%------------------------------------------------------------------------------
% File:        median_unbiased_estimator_stage2.m
%
...
function lame = median_unbiased_estimator_stage2_covid(y, x, kappa_vec)
    t_end = size(x, 1);
    stat = zeros(1, t_end - 2 * 4 + 1);
    w = diag(1 ./ (kappa_vec .^ 2));
    
    for i = 4:(t_end - 4)
        xr = [x, [zeros(i, 1); ones(t_end - i, 1)]];
        xi = inv(xr' * w * xr);
        b = xi * (xr' * w * y);
        s3 = sum(w * (y - xr * b) .^ 2) / (sum(diag(w)) - size(xr, 2));
        stat(i + 1 - 4) = b(end) / sqrt(s3 * xi(end, end));
    end
