...
% shiller data variables

real_stock_prices = 100*log(data_var(:,57));
real_dividends = 100*log(data_var(:,58));
real_earnings = 100*log(data_var(:,59));
pe_ratio = data_var(:,57)./data_var(:,59);
pd_ratio = data_var(:,57)./data_var(:,58);
cons_sent = data_var(:,60);
...
fof_start = find(dates_fof == date(1));
fof_end = find(dates_fof == date(end));

noise_equity_w = equity_w_flows(fof_start:fof_end);
noise_equity_w_3pc = equity_w_flows_3pc(fof_start:fof_end);
noise_equity_w_4pc = equity_w_flows_4pc(fof_start:fof_end);

noise_equity_w_sent = equity_w_flows_sent(fof_start:fof_end);
noise_equity_w_3pc_sent = equity_w_flows_3pc_sent(fof_start:fof_end);
noise_equity_w_4pc_sent = equity_w_flows_4pc_sent(fof_start:fof_end);

noise_equity_w_recent = equity_w_flows_recent(fof_start:fof_end);
noise_equity_w_3pc_recent = equity_w_flows_3pc_recent(fof_start:fof_end);
noise_equity_w_4pc_recent = equity_w_flows_4pc_recent(fof_start:fof_end);

noise_equity_w_recent_sent = equity_w_flows_recent_sent(fof_start:fof_end);
noise_equity_w_3pc_recent_sent = equity_w_flows_3pc_recent_sent(fof_start:fof_end);
noise_equity_w_4pc_recent_sent = equity_w_flows_4pc_recent_sent(fof_start:fof_end);


%noise_corpbond_w = corpbond_w_flows(fof_start:fof_end);
%noise_treas_w = treasury_w_flows(fof_start:fof_end);

ret_ex_d = log(data_returns_ex_d(fof_start:fof_end));
ret_cum_d = log(data_returns_cum_d(fof_start:fof_end));


clear data_var

load var_data_monthly.mat

date_m         = data_var(:,1);
shock_bs_ort = data_var(:,9);
shock_bs_ort(isnan(shock_bs_ort)) = 0;
yield_2y = data_var(:,7);
